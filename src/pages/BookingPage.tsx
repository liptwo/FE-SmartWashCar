...
import { vehiclesData, servicesData, Vehicle, ServiceItem } from "../data/mockData";
...
export default function BookingPage({ onBookingSuccess }: BookingPageProps) {
    // Booking state
    const [selectedVehicle, setSelectedVehicle] = useState<Vehicle>(vehiclesData[0]);
    const [selectedService, setSelectedService] = useState<ServiceItem>(servicesData[1]); // Gold Cao cấp as default in screen
    ...
    const handleConfirmBooking = () => {
        const confirmation = {
            ...
            durationMin: selectedService.id === 's3' ? 180 : selectedService.id === 's2' ? 75 : 45,
            technician: "Tùng Dương (Lead)",
            carImgUrl: selectedService.id === 's3'
        ...
        <div className="space-y-8 pb-16">
            ...
            <div className="flex items-center justify-between max-w-2xl mx-auto mb-8 relative">
                ...
                <div className="flex flex-col items-center gap-2 bg-[#F8FAFC] px-4">
                    ...
                    <span className="text-xs font-semibold text-slate-405">Thời gian</span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-12 gap-6">
                ...
                <div className="col-span-12 lg:col-span-4 space-y-4">
                    <div className="flex justify-between items-center">
                        <h3 className="text-sm font-bold text-slate-900 tracking-tight uppercase">1. Chọn phương tiện</h3>
                        <button
                            ...
                            return (
                                <div
                                    ...
                                    className={cn(
                                        ...
                                        isSelected ? "border-indigo-600 bg-indigo-50/15" : "border-slate-200"
                                    )}
                                >
                                    <div className={cn(
                                        "w-12 h-12 rounded-xl flex items-center justify-center transition-colors",
                                        isSelected ? "bg-indigo-100 text-indigo-700" : "bg-slate-50 text-slate-400 border border-slate-100"
                                    )}>
                                        <Car className="w-6 h-6" />
                                    </div>
                                    <div className="flex-1">
                                        <p className="text-xs font-bold text-slate-900">{veh.plate}</p>
                                        <p className="text-[11px] text-slate-405 font-medium mt-0.5">{veh.model}</p>
                                    </div>
                                    <div className={cn(
                ...
                <div className="col-span-12 lg:col-span-8 space-y-4">
                    <h3 className="text-sm font-bold text-slate-900 tracking-tight uppercase">2. Chọn gói dịch vụ</h3>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div>
                                        ...
                                        <h4 className={cn("text-xs font-bold mb-1.5 uppercase tracking-tight", isSelected ? "text-white" : "text-slate-800")}>
                                            {service.name}
                                        </h4>
                                        <p className={cn("text-[11px] leading-relaxed font-medium mt-0.5", isSelected ? "text-slate-300" : "text-slate-450")}>
                                            {service.description}
                                        </p>
                                    </div>

                                    <div className="mt-6">
                                        <p className={cn("text-base font-bold tracking-tight", isSelected ? "text-amber-400" : "text-indigo-600")}>
                ...
                <div className="col-span-12 space-y-4">
                    <h3 className="text-sm font-bold text-slate-900 tracking-tight uppercase">3. Thời gian thực hiện</h3>

                    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs space-y-6">

                        {/* Days view list */}
                        <div>
                            <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest mb-3">Chọn ngày thực hiện</p>
                            <div className="flex gap-3 overflow-x-auto no-scrollbar pb-1">
                                {days.map((day) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <span className="text-[10px] uppercase font-bold tracking-tight opacity-75">{day.dayOfWeek}</span>
                                            <span className={cn("text-base my-0.5 tracking-tight font-bold", isDaySelected ? "text-indigo-700" : "text-slate-800")}>
                                                {day.date}
                                            </span>
                                            <span className="text-[9px] uppercase tracking-wider opacity-70 font-bold">{day.label}</span>
                                        </button>
                                    );
                                })}
                            </div>
            ...
            <div className="fixed bottom-0 right-0 left-0 lg:left-64 bg-white border-t border-slate-200 px-6 py-4 flex justify-between items-center z-30 shadow-[0_-4px_24px_rgba(15,23,42,0.06)]">
                <div>
                    <div className="flex items-baseline gap-1">
                        <span className="text-xs text-slate-400 font-bold uppercase tracking-wide">Tổng cộng:</span>
                    ...
                    <div className="fixed inset-0 bg-slate-950/20 backdrop-blur-xs flex items-center justify-center z-50 p-4">
                        ...
                        >
                            <div className="flex justify-between items-center pb-2 border-b border-slate-100">
                                <h4 className="text-xs font-bold uppercase tracking-tight text-slate-800">Thêm xe mới vào hồ sơ</h4>
                                <button onClick={() => setIsAddCarOpen(false)} className="text-slate-450 hover:text-slate-600 text-xs font-bold cursor-pointer">Đóng</button>
                            </div>

                            <form onSubmit={handleAddNewCar} className="space-y-3 pt-1">
                                <div>
                                    <label className="block text-[10px] font-bold text-slate-450 uppercase tracking-wider mb-1">Biển kiểm soát (VD: 51H-999.99)</label>
                                    <input
                                        type="text"
                                        required
                                ...
                                <div>
                                    <label className="block text-[10px] font-bold text-slate-450 uppercase tracking-wider mb-1">Hãng xe & Dòng xe (VD: Toyota Fortuner)</label>
                                    <input
                                        type="text"
                                        required
                    ...
                    <div className="fixed inset-0 bg-slate-950/20 backdrop-blur-xs flex items-center justify-center z-50 p-4">
                        ...
                        >
                            ...
                            <div className="space-y-1.5">
                                <h4 className="text-sm font-bold text-slate-900 tracking-tight uppercase">Đặt lịch thành công!</h4>
                                <p className="text-xs text-slate-500 leading-relaxed font-semibold mt-1">
                                    Yêu cầu đặt lịch phục vụ xe <span className="font-bold text-slate-800">{selectedVehicle.plate}</span> vào lúc <span className="font-bold text-slate-800">{selectedTimeSlot} ({selectedDay}/06)</span> đã được lưu giữ trên hệ thống.
                                </p>
                            </div>
                            ...
                            <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200/60 text-left text-[11px] space-y-1.5 text-slate-600 font-semibold">
                                <p>• <span className="text-slate-400">Dịch vụ:</span> {selectedService.name}</p>
                                <p>• <span className="text-slate-400">Chi phí:</span> {formatCurrency(selectedService.price)}</p>
                                <p>• <span className="text-slate-400">Tích điểm:</span> +{selectedService.points} pts (Hạng Vàng)</p>
                            </div>
