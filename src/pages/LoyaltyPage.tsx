...
import { loyaltyHistory, LoyaltyHistoryItem } from "../data/mockData";
import { cn } from "../lib/utils";

export default function LoyaltyPage() {
    const [history, setHistory] = useState<LoyaltyHistoryItem[]>(loyaltyHistory);
    ...
    const handleClaimGift = () => {
        setClaimedReward("Cảm ơn bạn! Yêu cầu đổi voucher ưu đãi giảm 20% đã được gửi thành công. Hệ thống đã trừ 500 điểm tích lũy của bạn.");
        ...
        <div className="space-y-6">
            ...
            >
                <div className="flex items-center gap-6">
                    <div className="bg-amber-50 text-amber-800 px-4 py-3 rounded-xl border border-amber-200/50 flex items-center gap-2 shadow-xs">
                        ...
                        <span className="text-xs font-bold uppercase tracking-wider">GOLD</span>
                    </div>
                    <div>
                        <p className="text-3xl font-bold tracking-tight text-slate-900">1,240 pts</p>
                        <p className="text-xs font-medium text-slate-450 uppercase tracking-widest mt-0.5">Tài khoản hạng Vàng</p>
                    </div>
                </div>

                <div className="flex-1 max-w-md w-full">
                    <div className="flex justify-between mb-2">
                        <span className="text-xs font-bold text-slate-500 tracking-tight">Tiến trình lên Platinum</span>
                    ...
                    <p className="mt-2 text-[11px] text-slate-500 text-right font-medium">
                        Còn <span className="font-bold text-slate-800">760 điểm</span> / 15 lần dịch vụ để thăng hạng
                    </p>
                </div>
            </motion.section>
            ...
            <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-4 flex flex-col items-center justify-center gap-2 opacity-50 hover:opacity-75 transition-opacity shadow-xs">
                    <div className="bg-slate-50 text-slate-500 px-3 py-1 rounded-md text-xs font-bold border border-slate-200">MEMBER</div>
                    <span className="text-[10px] text-slate-400 font-bold tracking-tight">0 - 499 pts</span>
                </div>

                {/* Silver */}
                <div className="bg-white border border-slate-200 rounded-xl p-4 flex flex-col items-center justify-center gap-2 opacity-50 hover:opacity-75 transition-opacity shadow-xs">
                    <div className="bg-[#B5D4F4]/15 text-blue-600 px-3 py-1 rounded-md text-xs font-bold border border-blue-200/50">SILVER</div>
                    <span className="text-[10px] text-slate-400 font-bold tracking-tight">500 - 999 pts</span>
                </div>
                ...
                <div className="bg-white rounded-xl p-4 flex flex-col items-center justify-center gap-2 border-2 border-amber-400 ring-4 ring-amber-100 relative shadow-md">
                    <div className="bg-amber-50 text-amber-800 px-3 py-1 rounded-md text-xs font-bold border border-amber-200">GOLD</div>
                    <span className="text-[10px] text-amber-800 font-bold tracking-tight">1,000 - 2,499 pts</span>
                ...
                {/* Platinum */}
                <div className="bg-white border border-slate-200 rounded-xl p-4 flex flex-col items-center justify-center gap-2 opacity-50 hover:opacity-75 transition-opacity shadow-xs">
                    <div className="bg-indigo-50 text-indigo-700 px-3 py-1 rounded-md text-xs font-bold border border-indigo-100">PLATINUM</div>
                    <span className="text-[10px] text-slate-400 font-bold tracking-tight">2,500+ pts</span>
                </div>
            </section>
            ...
            >
                ...
                <p className="text-xs text-amber-800 leading-relaxed font-medium">
                    <span className="font-bold text-amber-950">240 điểm</span> sẽ hết hạn vào ngày <span className="font-bold text-amber-950">15/01/2026</span>. Hãy sử dụng điểm của bạn sớm để không bỏ lỡ các ưu đãi dọn xe đặc biệt nhé!
                </p>
            </motion.div>
            ...
            <section className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                    <div>
                        <h2 className="text-sm font-bold text-slate-900 tracking-tight">Lịch sử điểm thưởng</h2>
                        <p className="text-xs text-slate-400 font-medium mt-1">Biến động điểm tích lũy của bạn</p>
                    </div>
                            ...
                            >
                                ...
                                <div className="text-right">
                                    <span className={cn(
                                        ...
                                        item.type === 'expire' ? "text-slate-400 line-through" : ""
                                    )}>
                                        {item.points > 0 ? `+${item.points}` : item.points} pts
                                    </span>
                                    <p className="text-[9px] text-slate-400 uppercase font-bold tracking-widest mt-0.5">
                                        {item.type === 'earn' ? 'Tích lũy' : item.type === 'redeem' ? 'Đã dùng' : 'Hết hạn'}
                                    </p>
                                </div>
                            </motion.div>
                        ))
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="relative rounded-2xl overflow-hidden min-h-[160px] flex items-center p-6 border border-slate-200 group shadow-xs bg-slate-900">
                    ...
                    <div className="relative z-10 text-white max-w-[260px]">
                        <h3 className="text-sm font-bold text-white mb-1.5 tracking-tight uppercase">Đổi quà ngay</h3>
                ...
                <div className="bg-white rounded-2xl p-6 border border-slate-200 flex flex-col justify-center shadow-xs">
                    <div className="flex gap-4 items-start">
                        ...
                        <div>
                            <h3 className="text-sm font-bold text-slate-800 tracking-tight">Quyền lợi Platinum đang chờ bạn</h3>
