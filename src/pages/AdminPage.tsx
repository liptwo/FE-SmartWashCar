...
import { bookingsData, Booking } from "../data/mockData";
...
export default function AdminPage({
    ...
    const [dateRangeText, setDateRangeText] = useState<string>("12/10/2023 - 18/10/2023");
    ...
    const [newCustTier, setNewCustTier] = useState<Booking['customerTier']>("MEMBER");
    ...
    const filteredBookings = bookings.filter(b => {
        ...
        if (selectedStatusFilter !== "Tất cả trạng thái") {
            const matchStatus = b.status.toLowerCase() === selectedStatusFilter.toLowerCase().replace('_', '');
            // Handle status formatting discrepancies if any
            const formatStatus = b.status === 'PENDING' ? 'Pending' :
                b.status === 'CONFIRMED' ? 'Confirmed' :
                    b.status === 'IN_PROGRESS' ? 'In Progress' :
                        b.status === 'DONE' ? 'Done' : 'Cancelled';
            if (formatStatus !== selectedStatusFilter && b.status !== selectedStatusFilter) {
                return false;
            }
        }
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5 flex flex-col md:flex-row gap-4 items-end shadow-xs">
                ...
                <div className="flex-1 w-full space-y-1.5">
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Khoảng ngày</label>
                ...
                <div className="flex gap-2 w-full md:w-auto shrink-0 mt-3 md:mt-0">
                    ...
                    <button
                        className="flex-1 md:flex-initial px-5 py-2 bg-indigo-600 text-white rounded-lg text-xs font-bold hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-150/10 cursor-pointer"
                    >
                        Lọc dữ liệu
                    </button>
                </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-xs">
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {currentItems.length > 0 ? (
                                    ...
                                    >
                                        ...
                                        <td className="px-5 py-4">
                                            <div className="flex items-center gap-1.5">
                                                <span className="text-xs font-bold text-slate-850">{booking.customerName}</span>
                                        ...
                                        {/* Car Details */}
                                        <td className="px-5 py-4">
                                            <div className="text-xs font-bold text-slate-800">{booking.carPlate}</div>
                                            <div className="text-[10px] text-slate-450 font-bold uppercase mt-0.5">{booking.carModel}</div>
                                        </td>
                                        ...
                                        {/* Date/Time detail */}
                                        <td className="px-5 py-4">
                                            <div className="text-xs font-bold text-slate-800">{booking.timeStr}</div>
                                            <div className="text-[10px] text-slate-450 font-semibold mt-0.5">{booking.dateStr}</div>
                                        </td>
                                        ...
                                        <td className="px-5 py-4 text-center">
                                            <div className="inline-flex items-center gap-1 bg-slate-50 px-2 py-0.5 rounded-md border border-slate-200">
                                                <span className="text-[10px] font-bold text-slate-700">{booking.priority}</span>
                                                {booking.priority !== '--' && (
                                ...
                                <tr>
                                    <td colSpan={8} className="py-12 text-center text-gray-400 text-xs font-normal">
                                        Không tìm thấy booking nào phù hợp với bộ lọc hoặc từ khóa tìm kiếm.
                                    </td>
                                </tr>
                            )}
                        </tbody>
                ...
                <div className="px-5 py-4 flex items-center justify-between border-t border-slate-200 bg-white">
                    <p className="text-[11px] text-slate-400 font-bold uppercase tracking-wider">
                        Hiển thị {filteredBookings.length > 0 ? indexOfFirstItem + 1 : 0} - {Math.min(indexOfLastItem, filteredBookings.length)} trong số {filteredBookings.length} bookings
                    </p>
                    ...
                    <>
                        ...
                        >
                            ...
                            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
                                <div>
                                    <h3 className="text-xs font-bold uppercase tracking-wider text-slate-800">Chi tiết Booking</h3>
                                    <p className="text-xs text-indigo-600 font-bold leading-normal mt-0.5">#{activeBooking.id}</p>
                                </div>
                                <button
                                    onClick={() => setActiveBookingId(null)}
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-6">
                                ...
                                <div>
                                    <div className="aspect-video w-full rounded-2xl bg-slate-100 mb-3 overflow-hidden border border-slate-200 shadow-inner">
                                        <img
                                            src={activeBooking.carImgUrl || "https://images.unsplash.com/photo-1617814076367-b759c7d7e738?auto=format&fit=crop&q=80&w=600"}
                                            alt="Car in wash bay"
                                            className="w-full h-full object-cover select-none"
                                        />
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm font-bold tracking-tight text-slate-900">{activeBooking.carPlate}</span>
                                        <span className="text-xs text-slate-450 font-bold uppercase">{activeBooking.carModel}</span>
                                    </div>
                                </div>

                                {/* Customer card information box */}
                                <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200 space-y-3">
                                    <h4 className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Khách hàng</h4>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-xs font-bold text-slate-850">{activeBooking.customerName}</p>
                                            <p className="text-[11px] text-slate-455 font-semibold mt-0.5">{activeBooking.customerPhone}</p>
                                            <div className="mt-1.5">
                                                <Badge variant={getTierVariant(activeBooking.customerTier)}>{activeBooking.customerTier} MEMBER</Badge>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-3">
                                    <h4 className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Thông tin dịch vụ</h4>
                                    <div className="space-y-2 border-t border-slate-150 pt-2 text-xs">
                                        <div className="flex justify-between items-center py-1.5 border-b border-dashed border-slate-100">
                                            <span className="text-slate-450 font-bold uppercase text-[10px] tracking-wider">Dịch vụ chính</span>
                                            <span className="font-bold text-slate-800">{activeBooking.serviceName}</span>
                                        </div>
                                        <div className="flex justify-between items-center py-1.5 border-b border-dashed border-slate-100">
                                            <span className="text-slate-450 font-bold uppercase text-[10px] tracking-wider">Thời gian dự kiến</span>
                                            <span className="font-bold text-slate-800">{activeBooking.durationMin || 60} phút</span>
                                        </div>
                                        <div className="flex justify-between items-center py-1.5 border-b border-dashed border-slate-100">
                                            <span className="text-slate-450 font-bold uppercase text-[10px] tracking-wider">Kỹ thuật viên</span>
                                            <span className="font-bold text-indigo-650">{activeBooking.technician || "Tùng Dương (Lead)"}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <h4 className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Lịch trình trạng thái</h4>
                                    <div className="relative pl-6 space-y-5 before:content-[''] before:absolute before:left-[7px] before:top-2 before:bottom-2 before:w-[1px] before:bg-slate-200">
                                        ...
                                        <div className="relative">
                                            ...
                                            <p className="text-xs font-bold text-slate-900">Đã đặt lịch</p>
                                            <p className="text-[10px] text-slate-450 font-bold mt-0.5">{activeBooking.dateStr} - {activeBooking.timeStr}</p>
                                        </div>
                                        ...
                                        <div className={cn(
                                            ...
                                            <span className={cn(
                                                ...
                                                activeBooking.status === 'PENDING' ? "bg-slate-200" : "bg-emerald-600"
                                            )}></span>
                                            <p className="text-xs font-bold text-slate-900">Xác nhận</p>
                                            <p className="text-[10px] text-slate-450 font-bold mt-0.5">
                                                {activeBooking.status === 'PENDING' ? 'Chưa xác nhận' : 'Đã phê duyệt hồ sơ'}
                                            </p>
                                        </div>
                                        ...
                                        <div className={cn(
                                            "relative",
                                            ['PENDING', 'CONFIRMED'].includes(activeBooking.status) ? "opacity-45" : ""
                                        )}>
                                            <span className={cn(
                                                "absolute -left-7 top-1 w-3.5 h-3.5 rounded-full ring-4 ring-white border-2 border-white",
                                                ['PENDING', 'CONFIRMED'].includes(activeBooking.status) ? "bg-slate-200" : "bg-emerald-600"
                                            )}></span>
                                            <p className="text-xs font-bold text-slate-900">Bắt đầu thực hiện</p>
                                            <p className="text-[10px] text-slate-450 font-bold mt-0.5">
                                                {['IN_PROGRESS', 'DONE'].includes(activeBooking.status) ? 'Đang rửa / vệ sinh chi tiết' : 'Chưa cập nhật'}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {activeBooking.notes && (
                                    <div className="space-y-1.5">
                                        <h4 className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Ghi chú từ khách hàng</h4>
                    ...
                    <div className="fixed inset-0 bg-slate-950/20 backdrop-blur-xs flex items-center justify-center z-50 p-4">
                        ...
                        >
                            <div className="flex justify-between items-center pb-2 border-b border-slate-100">
                                ...
                                <button onClick={() => setIsNewBookingOpen(false)} className="text-slate-450 hover:text-slate-600 text-xs font-bold cursor-pointer">Đóng</button>
                            </div>

                            <form onSubmit={handleCreateNewBookingSubmit} className="space-y-3 text-xs pt-1">
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        <label className="block text-[10px] font-bold text-slate-450 uppercase tracking-wider mb-1">Tên khách hàng</label>
                                        <input
                                            type="text"
                                            required
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-bold text-slate-450 uppercase tracking-wider mb-1">Hội viên Hạng</label>
                                        <select
                                            value={newCustTier}
                                            onChange={(e) => setNewCustTier(e.target.value as Booking['customerTier'])}
                                ...
                                <div>
                                    <label className="block text-[10px] font-bold text-slate-450 uppercase tracking-wider mb-1">Số điện thoại</label>
                                    <input
                                        type="tel"
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        <label className="block text-[10px] font-bold text-slate-450 uppercase tracking-wider mb-1">Biển kiểm soát</label>
                                        <input
                                            type="text"
                                            required
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-bold text-slate-450 uppercase tracking-wider mb-1">Dòng xe</label>
                                        <input
                                            type="text"
                                            required
                                ...
                                <div>
                                    <label className="block text-[10px] font-bold text-slate-450 uppercase tracking-wider mb-1">Chọn loại dịch vụ</label>
                                    <select
                                        value={newService}
                                    ...
                                    >
                                        ...
                                        <option value="Đánh bóng & Hiệu chỉnh sơn">Đánh bóng & Hiệu chỉnh sơn</option>
                                        <option value="Rửa gầm nâng cấp">Rửa gầm nâng cấp</option>
                                        <option value="Combo Chăm sóc Nội thất">Combo Chăm sóc Nội thất</option>
                                    </select>
                                </div>
