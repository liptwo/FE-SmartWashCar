...
import type { AdminPromotion } from '@/data/admin-promotions'
...
export function CreatePromotionDrawer({ onClose, open }: CreatePromotionDrawerProps) {
  ...
  return (
    <>
      ...
      <aside className="fixed inset-y-0 right-0 z-50 flex w-full max-w-[450px] flex-col border-l border-outline-variant bg-surface shadow-2xl">
        ...
        <div className="custom-scrollbar flex-1 space-y-6 overflow-y-auto p-6">
          ...
          <label className="grid gap-3 text-sm font-medium leading-4 text-on-surface">
            Loại khuyến mãi
            <select className="h-12 rounded-lg border border-outline-variant bg-surface px-4 text-base outline-none focus:border-primary focus:ring-2 focus:ring-primary/15">
          ...
          <div className="rounded-lg border border-info/10 bg-info/5 p-4 text-xs leading-5 text-on-surface-variant">
            Chương trình sau khi lưu có thể được gửi notification trực tiếp tới khách hàng thuộc tier đã chọn.
          </div>
        </div>
